...
export class LdapNotificationTask {
    ...
    async run(): Promise<void> {
        ...
        if (missingUsers.length === 0) {
            this.logger.info(
                'No users with tag `ldap-not-found` found, nothing to notify',
            );
            return;
        }
    ...
    private async buildTeamBatches(
        ...
        for (const user of missingUsers) {
            ...
            if (teams.length === 0) {
                this.logger.warn(
                    `User ${stringifyEntityRef(
                        user,
                    )} has no memberOf teams, skipping`,
                );
                continue;
            }
    ...
    private async notifyTeam(batch: TeamNotificationBatch): Promise<void> {
        const listings = batch.productListings.map(
            listing =>
                `${listing.metadata.name} (ID: ${
                    listing.metadata.annotations?.['redhat.com/pyxis-id'] ?? '-'
                })`,
        );
