...
- The backend reads **which clusters/namespaces to query from an annotation
  ...
  hardcoded list — but which *clusters exist at all* (their URLs/tokens) **is**
...
- **There is no per-user ownership/RBAC filtering of *which* resources are
...
**Short answer: identity is used for *authentication*, not for *authorization/filtering*. There is no "my resources" view.**
...
and are used for exactly three things — none of which filter *which*
resources come back:
...
### 3.4 What exists in Backstage that this *could* be built on
...
- `@backstage/plugin-permission-node`/`-common` — the konflux backend
  ...
  for the *stock* Kubernetes plugin, not for `konflux`.
...
1. **Smaller change — rely on K8s RBAC + impersonation, drop the namespace
   ...
   constrain what they can `list` *if you call `list` across all namespaces*
...
- The only caching is a 30-second in-process TTL cache
  ...
  lookups — purely to deduplicate near-simultaneous requests for the *same
  entity*, not a data warehouse. A K8s API client cache also exists in
...
viewing. If your feature-overview needs to *search across* or *join*
